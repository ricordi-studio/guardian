...
const 配るもの = new Set([
  '保持一覧.json',      /* ★塊の既定の保持一覧(2026-09-03)。現場は config の retain で足せる */
   ...
   *   ★★★--束も が 使えない。 */
  '目録.json',
  /* ★撤去が 束を 移した後で 落ちた時の【回復役】(27.88)── ★★束の 外へ 写して 使う。 */
  '撤去を終える.mjs',
]);
const 現場のもの = new Set([
  ...
  /* ★install が【束の中の 古い 受領証】を 出す先(27.71)。★★現場の根の 直下に 作る。
   *   ★★★中身は 見ない・台帳にも 載せない ── だから 配ってはいけない(配ると 人の物を 踏む)。 */
  'guardian-隔離-',
...
/* ★★★【知らない物を 黙って 上書きしない】(27.64、2026-09-05、@codex 07:07)。
 *
 *   ★@codex の実測(公開 27.63):「pull 前の 同名 利用者ファイルが
 *     ★★出口0・『変わるものなし』のまま 上書きされる」──
 *   ★★★これは 束の削除とは【別】の、公開中の データ損失。
 *
 *   ★pull は 受領証を 書く前に、そこに 在る物を【無効にする】(退避か 削除)。
 *   ★★その時、在る物が 自分の受領証か どうかを 見ていなかった。
 *   ★★★= 人が そこに 置いた物を、名前が 同じというだけで 消していた。
 *
 *   ★見るのは【形】── sha / 正本 / at で 在るか。★★形が 違えば 触らず 止まる。
 *   ★★★形が 合っても「pull が 書いた」証明には ならない(★@codex 07:03)──
 *   だが【明らかに 人の物】を 消す事だけは、これで 止まる。 */
function 受領証の形か(道) {
  let t = null;
  try { t = fs.readFileSync(道, 'utf8'); } catch (_) { return true; }   /* ★読めない = 判じない */
  try {
    const j = JSON.parse(t);
    return !!j && typeof j.sha === 'string' && typeof j.正本 === 'string' && typeof j.at === 'string';
  } catch (_) { return false; }
}
function 知らない物なら止める() {
  if (!fs.existsSync(受領証の先)) return;
  if (受領証の形か(受領証の先)) return;
  console.error('✗ ' + 受領証の先 + ' に【受領証では ない物】が 在ります。');
  console.error('  ★pull は ここに 受領証を 書きます ── ★★このままだと あなたの物を 消します。');
  console.error('  ★★★中を 見て、要らなければ 消してから もう一度 打ってください'
    + '(★要る物なら、別の名前へ 移してください)。');
  process.exit(2);
}
function 受領証を無効にする() {
...
/* ── ①' 食い違いを、正本と突き合わせて仕分ける(クローンが済んでから) ── */
{
  ...
  if (食い違い === null) {
    ...
    if (この現場の直り.length) {
      ...
      if (強く) {   /* ★同じ問いは1箇所で読む(24.2)── ここは :107 の 強く を使う */
...
/* ★「撤去された」と言い切らない(2026-09-03、外の監査役の指摘)。
 *   ★★こちらから見えるのは【正本に無い】ことだけである。
 *   「上流で消された」と「上流にまだ無い(正本が古い)」は別の事実で、★★★人が取る手が逆になる
 *   (前者は追随 / 後者は差し戻し)。だから見えたままを言う。 */
if (全消える.length) console.log('消えるもの: ' + 全消える.join(', ')
  + '(★正本に在りません ── 上流で消されたのか、上流がまだ古いのかは、ここからは分かりません)');
...
/* ★【降格】と【エンジンの消失】で止める(2026-09-03、外の監査役が実測で見つけた)。
 *
 *   ★★実測(直す前): 21.2 の配布先で README の案内どおり `node guardian/pull.mjs` を回すと
 *   ★★★版が 21.2 → 13.24 に下がり、5本(保持一覧.json / 道.cjs / 書き手.cjs / 外す.mjs / 台帳.mjs)が消え、
 *   **出口0・「✓ 取り直しました」** で終わった。全部が緑である。
 *
 *   ★消えた5本は飾りではない ── SPEC が「現場へ書く唯一の口」「git から道を取る唯一の口」
 *   「導入・撤去の土台」と名指ししている層である。
 *   ★★しかも【台帳】は現場に残るのに、読み手の 外す.mjs が消えるので、
 *   ★★★取り直した後は【外す手段そのものが無くなる】。
 *
 *   ★道具は「版: 旧 → 新」と並べて出すだけで、大小を比べていなかった。
 *   ★★比べる。下がるなら止める。エンジンが消えるなら止める。
 *   ★★★どうしても取るなら --降格を承知 ── ただし何が消えるかを先に出してからにする。
 *   (★24.0 まで ここは --force と書いていた。★★画面は直っていたのに注釈だけが残り、
 *   ★★★次に直す人が「--force で降格できる」と読む形になっていた ── 会議で @kozo が見つけた) */
const 版を数に = (v) => {
  const m = String(v).match(new RegExp("^(\\d+)\\.(\\d+)"));
...
};
{
  ...
  /* ★危ない所は【1つの所】から言う(27.24、2026-09-03、@kozo が実測)。
 *
 *   ★★事故: この塊は 断りの文の中で「★確かめ方: --check(取らずに差だけ見る)」と案内する。
 *   ★★★だが --check は この塊ごと 飛ばしていたので、
 *   版が下がる事も、外す手段が消える事も **1文字も 出なかった**。
 *   実測(@kozo): pull は13行 出し、--check は5行。危ない語は どれも 0件。
 *
 *   ★「先に --check で確かめて」と言われた人が見るのは、**いちばん静かな方**だった。
 *   ★★同じ場面・2つの口・違う情報 ── 今日ずっと出ている形。
 *   ★★★文を2つ持たない ── ここから 両方が 出す。 */
  const 危ないと言う = () => {
    console.error('');
    if (下がる) console.error('  ★★版が下がります: ' + 旧版 + ' → ' + 新版);
    if (消えるエンジン.length)
      console.error('  ★★★配ると宣言している物が消えます(' + 消えるエンジン.length + '件): '
        + 消えるエンジン.join(', '));
    console.error('  ★正本がまだ古い(こちらが先行している)のか、上流で本当に消されたのかは、'
      + 'ここからは分かりません。');
    console.error('  ── 外す.mjs が消えると、台帳が現場に残ったまま【外す手段が無くなります】。');
  };
  /* ★下見(--check)でも、同じ危なさを 同じ文で 言う(27.24)。
   *   ★★取らないので 出口は 0 のまま ── ★★★だが「出口0 = 安全に更新できる」ではない と はっきり書く。 */
  if ((下がる || 消えるエンジン.length) && 見るだけ) {
    console.error('');
    console.error('★ここから先は【危ない更新】です ── このまま取ると、いまより【後ろ】へ動きます');
    危ないと言う();
    console.error('  ★★これは --check なので【取っていません】(現場は 1バイトも 動いていません)。');
    console.error('  ★★★出口0 は「安全に更新できる」ではなく「取らなかった」という意味です。');
  ...
  if ((下がる || 消えるエンジン.length) && !見るだけ && !降格を承知) {
    ...
    危ないと言う();
    console.error('  ★★確かめ方: node guardian/pull.mjs --check'
      + '(取らずに差だけ見る ── ★★★この断りと 同じ事を 言います)');
    console.error('  ★★★それでも取るなら: node guardian/pull.mjs --降格を承知');
