 ...
 *   ★★★測った事にしてはいけない。だから 止める枝より 後ろに置く。 */
  ...
  if (unknown.length || warn.length) {
    ...
    const reason = '合否は止めるほどではありませんが、**測れていないもの**があります。'
      ...
      /* ★人向けにも【何を許していないか】を出す(27.41)── ★★機械の欄だけに書くと、
       *   ★★★人は「止まらなかった＝進んでよい」と読む。出どころは verdict の宣言 1つ。 */
      + (v && v.不明の契約
        ? String.fromCharCode(10) + '★不明のとき ここが許すのは【' + v.不明の契約.意味 + '】── '
          + '★★次は許していません: ' + (v.不明の契約.許していない || []).join(' / ')
        : String.fromCharCode(10) + '★この合否は 契約を 名乗っていません(古い verdict かもしれません)'
          + ' ── ★★分からない以上、撤去も 配布も しないでください');
    ...
    process.stdout.write(JSON.stringify({
      ...
      /* ★契約は verdict.mjs が 宣言する ── ★★ここは【読む】だけ(27.41、@codex 02:17)。
       *   ★★★写すと、写した瞬間から 古くなる。読めなければ【読めなかったと 言う】。 */
      不明の契約: (v && v.不明の契約) || { 意味: '★この合否は 契約を 名乗っていません(古い verdict かもしれません)',
        許していない: ['★分かりません ── ★★分からない以上、撤去も 配布も しないでください'] },
