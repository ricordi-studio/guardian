...
/* @黙る道: 全部 ── 輪の中に居ない現場では言う事が無い。輪の中で wait を呼ばずに終えた時だけ止める */
...
const os = require('os');
/* ★この門は【配られる】が、頼る物は配られない(2026-09-03、実測で見つけた)。
 *   ★★pull.mjs の配布宣言は hooks を【フォルダごと】指すので、この1本も配布先へ行く。
 *   ところが 印の場所.cjs は `.guardian/` に在る = 【現場のもの】なので、★★★配られない。
 *   実測: 配布の形の写しで叩いたら Cannot find module で落ちた。
 *   ★いま実害は無い(install はこの門を登録しないので誰も呼ばない)。
 *   ★★だが【配る物の中に、配布先では必ず落ちる物】が在るのは、それ自体が穴である。
 *   ★★★門は落ちてはいけない ── 頼る物が無ければ「会議していない」と同じ扱いで通す。 */
let 印の場所 = null, 帳の場所 = null;
try {
  ({ 印の場所, 帳の場所 } = require(path.join(__dirname, '..', '.guardian', '印の場所.cjs')));
} catch (_) { /* 会議の道具が無い現場 ── この門は何もしない */ }
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    /* ★頼る物が無ければ、会議していないのと同じ ── 何もせず通す(2026-09-03) */
    if (!印の場所) return pass('通した(会議の道具が無いので、この門は働かない)');
    const 印 = 印の場所(根);
