 ...
 *   **数だけ出す**(9.72 の3段の1段目)。消せない数として残り、次に見た人が判断する。 */
{
  ...
  if (理由たち.length > 2) {
    ...
    let 最多 = 0;
    for (const n of 数.values()) if (n > 最多) 最多 = n;
    if (最多 > 1) {
      const 割 = Math.round((最多 / 理由たち.length) * 100);
      console.log("  ・同じ理由が " + 最多 + "/" + 理由たち.length + " 件(" + 割 + "%)に並んでいます ── 門は【答えが在るか】しか見ていません。**問いに答えているか**は、書いた本人にしか分かりません");
    }
  }
}
