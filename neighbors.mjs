...
/* ★印は【バイトのまま】取る(27.20 で 27.19 を戻した、2026-09-03)。
 *
 *   ★★27.19 で「作業木は CRLF・commit は LF だから 印が合わない」と読んで正規化したが、
 *   ★★★**外れだった**。@kozo の見立て(答えた後に中身を直して、そのまま押した)が正しかった。
 *
 *   ★決め手(1ab2723 で実測):
 *     保存された印 … 14csu2v-11459
 *     LF から取ると … 1d2yweq-11278
 *     CRLF から取ると … zwk9h-11561
 *   ★★**どちらとも 違う** ── 改行の話ではなく、答えた時の中身が もう無い、という話だった。
 *
 *   ★★★正規化には【costがある】── 印の取り方を変えると、
 *   **既に配った現場の答えが 全部 無効になる**(実測: 同じコミットで ✗5 → ✗8)。
 *   ★効かない上に高いので、戻した。
 *
 *   ★★直すべきは道具ではなく【押し方】だった ── 27.20 の WHY に規律として書いた。 */
const 印を取る = (s) => {
  let h = 2166136261;
