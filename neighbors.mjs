...
/* ★印は【改行を揃えてから】取る(27.29、2026-09-04)── ★★27.19 を 入れ直した。
 *
 *   ★★★私は 27.19 で これを入れ、27.20 で 取り下げた ── ★取り下げが 誤りだった。
 *
 *   ★★決め手(27.28 を押した後、clone の門が 赤くなった時の実測):
 *     私の木  … hooks/stop.js 19467バイト / ★CR 192個
 *     ★★★clone  … 19275バイト / CR 0個(同じコミット)
 *     保存された印 … ★私の木の値と 一致 / clone で取り直すと 違う
 *   → ★★答えは【私の木でしか】通らない。他の席や CI が clone すると 赤くなる。
 *
 *   ★27.20 で「どちらとも違う」と測ったのは、★★その時 既に 私が その後 編集していたから。
 *   ★★★同じ物を 2度 測って、2度目だけを 信じた ── 測り方の失敗だった。
 *
 *   ★費用は 承知している: 印の取り方が変わるので、
 *   ★★既に配った現場の答えは【一度だけ】無効になる(1回 答え直せば 済む)。
 *   ★★★効かない直しではなく、**効く直し**だった ── clone の門で 確かめる。
 *   (★selfcheck の指紋は もう改行を正規化している ── ここだけ 揃っていなかった) */
const 印を取る = (s0) => {
...
/* ---------- ④.6 コードが変わったら、それを名指ししている【案内】も近傍(2026-09-01) ----------
 *
 * ★なぜ在るか: 配布先で「案内が古い」が1日に3件 出た(在るのに書かれていない口 /
 *   直したのに残る「失敗0」/ 数え直したのに古い回数)。★★どれも人が読めば分かるが、
 *   ★★★コードを変えた回に【問われない】ので、誰も見に行かなかった。
 *
 * ★記録(CHANGELOG / WHY / 引き継ぎ)は【過去の話】なので古くならない ── 問うてはいけない。
 *   ★★問うべきは【いまを説明している案内】だけ。実測: この塊では 1つの道具を
 *   名指しする .md が 8〜12件 在り、全部 問うと同じ理由の貼り付けが増える(今夜 83%)。
 *   ★★★だから既定は4件に絞り、現場が宣言で差し替えられるようにする(39条)。 */
{
  const 案内 = Array.isArray(N.guide_docs) && N.guide_docs.length ? N.guide_docs
    : ['SPEC.md', 'README.md', 'install.md', 'docs/CODEMAP.md'];
  for (const [cf] of changed) {
    if (!corpus.has(cf)) continue;                 /* コードだけ(ノートは④が見る) */
    const 名 = cf.split('/').pop();
    for (const d of 案内) {
      const t = read(d);
      if (!t || !t.includes(名)) continue;
      const key = d + '::案内:' + 名;
      if (need.has(key)) continue;
      need.set(key, {
        記号: '案内:' + 名, 場所: d, 環: 1, 根: 'proc:' + cf,
        きっかけ: d + ' が ' + 名 + ' を名指ししている(中身が変わったので、案内が古くないか)' });
    }
  }
}
...
for (const x of 一覧) {
  ...
  if (a.判定 === '触れた') {
     ...
     *   ★★記号の名(「案内:X」「子として回す:X」)は差分に出ない。
     *   ★★★そこは記号ではなく【ファイルの側で見る】── その場所が差分に在れば「触れた」。
     *   (2026-09-01、④.6 を入れた直後に自分で踏んだ。触ったのに差戻になった) */
    const 疑似 = /^(案内|子として回す):/.test(x.記号);
    const ok = 疑似 ? [...changed.keys()].includes(x.場所)
                    : (touched.has(x.記号) || x.key.startsWith('*::'));
    if (!ok) 落ち.push('「触れた」と答えたのに差分に無い: '
      + (疑似 ? x.場所 : x.記号) + '(直したなら差分に入るはず)');
    continue;
  }
