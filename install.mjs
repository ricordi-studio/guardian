  ...
  try {
    ...
    const 歩く = (d, 親) => {
      ...
      for (const x of es) {
        ...
        /* ★★★名前だけで 通しては いけない(27.63、@codex 06:45)。
         *   ★27.61 は 名前が 合えば 無条件で 照らさなかった ── ★★人が
         *   guardian/.guardian/pulled.json に【自分の物】を 置いたら、そのまま 巻き込む。
         *   ★★★私は 一つの穴を 塞いで、狭い穴を 一つ 開けていた。
         *   ★だから【中身が その物か】まで 見る:pull が 書く形(sha / 正本 / at)で 在るか。
         *   ★★合わなければ 通さない ── ★★★「知らない物」として 断る側に 倒す。 */
        if (目録 && 目録.走行が作る.has(名)) {
          let 形が合う = false;
          try {
            const j = JSON.parse(中 == null ? '' : 中);
            形が合う = !!j && typeof j.sha === 'string' && typeof j.正本 === 'string' && typeof j.at === 'string';
          } catch (_) { 形が合う = false; }
          if (形が合う) continue;
          外れ.push(名 + '(走行が作る物の 名前だが、中身が その形では ない)');
          continue;
        }
        if (!目録) return;
