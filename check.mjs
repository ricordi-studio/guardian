...
const 知っている口 = ['--口一覧', '--片方だけ', '--名指しされていない', '--宣言の外', '--呼び先が地図に無い', '--逃げ道の理由', '--冷たい順', '--予想', '--root', '--selectors', '--tighten'];
...
const problems = [];
const notes = [];
/* ★【計器】と【判定】を分ける(27.0、2026-09-03、@codex 13:36)。
 *
 *   ★★事故: notes は全部 ✓ で印字していた。だが その中には
 *   「9件が片方しか機械に見えていない」「306個のうち地図が名指しするのは32個」のような
 *   ★★★【測れていない量】が混ざっていた ── 自分で「落としません」と書きながら、✓ を付けていた。
 *
 *   ★@codex:「測定不足は state=unknown または明示的 partial とし、
 *   exit0／greenCount／verdict=pass へ流さない契約に」。
 *
 *   ★★ここでは出口は変えない(これらは元から落とさない物なので)。
 *   ★★★変えるのは【印と勘定】── ✓ ではなく 計 で出し、通過の数に混ぜない。 */
const 計器 = [];
...
if (!map) {
   ...
   * (2026-08-18の監査で発覚。「済んだら消す」と書いてあるのに地図だけ生き残っていた)。 */
  {
    ...
    if (生成物.length) 計器.push(`地図が名指す .guardian/ の ${生成物.length} 件は生成物なので実在を測っていません: ${生成物.join(', ')}`);
  }
   ...
   *   ── 接点を書こうとして、**半分だけ機械に見えている**状態がこれである。 */
  {
    ...
    if (片方だけ.length) {
      ...
      計器.push('地図の項 ' + (塊.length - 1) + ' 件のうち、片方しか機械に見えていない項が ' + 片方だけ.length + '件: '
        + 片方だけ.slice(0, 5).join(' / ')
   ...
   * ★測れないときは黙らない ── 「見ていません」と出す。 */
  {
    ...
    if (r.status !== 0 || !名前.size) {
      計器.push("この現場の実装のうち、地図が名指ししている割合は**見ていません**"
      ...
      計器.push("この現場の実装 " + 名前.size + " 個のうち、**地図が名指ししているのは "
        + 地図に在る.size + " 個**(" + 割 + "%)"
 ...
 *
    ...
    計器.push("検査が読んでいないコードが在るかは**見ていません**(" + String(e && e.message).slice(0, 80) + ")");
  }
  ...
  const 言う = (名, 一覧, 説明) => {
    if (!一覧.length) return;
    計器.push("**" + 名 + " が " + 一覧.length + " ファイル**あります: "
  ...
  if (落とした宣言外 || 落とした無視) {
    ...
    計器.push("★この数は、" + 内訳.join(" / ") + " を**分母から外して**出しています ── "
 ...
 * ★落とさない ── 索引に全部は載らない。**数と、取り出す口だけ**(9.72 の3段)。 */
{
  ...
  try {
    ...
    計器.push("地図の呼び先は**見ていません**(" + String(e && e.message).slice(0, 80) + ")");
  }
  ...
  if (呼び先.length) {
    計器.push("**地図に載っているファイルが呼んでいるのに、地図に無いファイルが " + 呼び先.length + " 件**あります: "
 ...
 * ★判定にはしない(見本を書けない検査も在る)。★★数だけ出す ── 落とさない。 */
{
  ...
  if (対象.length) {
    ...
    計器.push("式で探す検査 " + 対象.length + " 本のうち、【見本(probe)】を持つのは "
      + 見本あり.length + " 本(" + 割 + "%)"
  ...
  }
  {
    ...
    if (理由たち.length > 2) {
      ...
      if (最多 > 1) {
        ...
        計器.push("逃げ道の理由: 種類 " + 数.size + " 通り / 【いちばん多い理由】が " + 最多 + "/" + 全
 ...
 * ★git が無ければ何も言わない(7条 ── 測れないものを言わない)。 */
{
  ...
  if (r.status === 0) {
     ...
     *   ★分けないと、配布先の総量は【常に塊の版数】を見ていることになる。
     *   ★★★正本では 塊 = 根なので、この分けは働かない(そこでは全部が「この現場」)。 */
    const 塊は別 = path.resolve(KIT) !== path.resolve(ROOT);
    const 塊の下 = 塊は別 ? path.relative(ROOT, path.resolve(KIT)).split(path.sep).join("/") : "";
    let 足 = 0, 引 = 0, 個 = 0, 塊足 = 0, 塊引 = 0, 塊個 = 0;
    let 未追跡個 = 0, 未追跡足 = 0;
    /* ★見ない場所は、追跡済みの側と同じにする(git が既に除いている分に揃える) */
    const この検査が見ない2 = new RegExp("(^|/)(\\.git|node_modules)(/|$)");
    for (const 行 of String(r.stdout || "").split(/\r?\n/)) {
      const m = 行.match(/^(\d+|-)\t(\d+|-)\t(.*)$/);
      if (!m) continue;
      個++;
      const 足1 = m[1] === "-" ? 0 : Number(m[1]);
      const 引1 = m[2] === "-" ? 0 : Number(m[2]);
      足 += 足1; 引 += 引1;
      if (塊の下 && (m[3] === 塊の下 || m[3].startsWith(塊の下 + "/"))) {
        塊個++; 塊足 += 足1; 塊引 += 引1;
      }
    }
    /* ★【未追跡のファイル】も数える(2026-09-03、★★外の監査役が実測で見つけた)。
     *
     *   ★★★実測(直す前): 4000行の新規ファイル2本を置いても、この計器は**1行も出さなかった**。
     *   そのあと追跡済みを1行 変えると「1 ファイル / +1 -0 行」── ★8001行が「+1」になる。
     *
     *   ★★原因: git diff --numstat HEAD は**未追跡を出さない**。
     *   ★★★この計器の由来は「8917行が未コミットだった ── 誰も一度も数えていない」である。
     ...
     *   ★★正しい呼び方をこの現場は既に持っていて、ここだけ使っていなかった。 */
    {
      ...
      if (!u.落ちた) {
        for (const rel of u.道) {
          if (この検査が見ない2.test(rel)) continue;
          let n = 0;
          try {
            const t = fs.readFileSync(path.join(ROOT, rel), "utf8");
            if (t.indexOf(String.fromCharCode(0)) >= 0) continue;   /* 文字の入れ物ではない */
            n = t.length ? t.split(/\r?\n/).length - (t.endsWith(String.fromCharCode(10)) ? 1 : 0) : 0;
          } catch (_) { continue; }
          個++; 未追跡個++; 足 += n; 未追跡足 += n;
          if (塊の下 && (rel === 塊の下 || rel.startsWith(塊の下 + "/"))) { 塊個++; 塊足 += n; }
        }
        if (u.読めなかった.length) problems.push("★未追跡の道 " + u.読めなかった.length
          + "件が UTF-8 として読めません ── ★★この計器は、その分を数えていません");
      }
    }
    if (個) {
      const 内訳 = 塊個
        ? "(★うち塊(" + 塊の下 + ") " + 塊個 + " ファイル / +" + 塊足 + " -" + 塊引
          + " 行 ── ★★取り直しで動く分。**この現場が書いたのは " + (個 - 塊個) + " ファイル / +"
          + (足 - 塊足) + " -" + (引 - 塊引) + " 行**)"
        : "";
      const 未追跡の内訳 = 未追跡個
        ? "(★うち **まだ git に入れていない新しいファイル** " + 未追跡個 + " ファイル / +" + 未追跡足 + " 行)"
        : "";
      計器.push("まだコミットしていない変更: " + 個 + " ファイル / +" + 足 + " -" + 引 + " 行" + 内訳 + 未追跡の内訳
        + " ── ★**大きいことは悪ではありません**(一晩で作れば大きくなります)。"
        + "★★**ただし、誰も数えていないと【存在ごと見えなくなります】**(落としません。上限も置いていません)");
    }
  }
}
/* ★【いつ触ったか】で並べる(2026-08-31、読み手の整理 + 配布先の実測)。
 *
 * ★マクロの計器は「どこが」ではなく「いつもと違う」を言う。★★外れ値は3つ:
 *     ① 速すぎる … 規則の無断変更(実測: 依頼主が版数の進み方で見抜いた)
 *     ② 遅すぎる … ★盲点・検査の不在(実測: 30日で3回しか触っていない所に、暗号の3つ目が居た)
 *     ③ 直した直後 … ★★数え漏れ・記録の腐り(実測: 公開口の穴 / 直したから腐った数)
 * ★★①は人が気づく(数字が目に入る)。★★★②③は誰も見ていない ── そこを出す。
 ...
 * ★git が無ければ何も言わない(7条)。 */
{
  ...
  if (!r.落ちた) {
    ...
    const 場所 = new Map();
    /* ★2026-09-03: ここは 18.0 で【壊れていた】── 口を 道を取る() に替えたとき、
     *   ★★この行だけ r.stdout を読んだままにした。その欄はもう無いので、
     *   ★★★ループが1度も回らず、場所は常に空になり「冷たい順」の行が消えていた。
     *   ★誰も捕まえなかった: 出力が1行 減っただけで、赤も未測も出ない。
     * ★★だから会議(@codex)の保存則を、この下に入れた ──
     *   ★★★入った道は【使われた】か【理由付きで外れた】かの、どちらかでなければならない。 */
    const 使った道 = new Set();
    const 外した道 = new Set();
    for (const t of r.道) {
      if (この検査が見ない.test(t)) { 外した道.add(t); continue; }   /* ★この検査だけのもの(隣から借りない・4回目) */
      const 頭 = t.includes("/") ? t.slice(0, t.indexOf("/")) : ".";
      if (!場所.has(頭)) 場所.set(頭, 0);
      使った道.add(t);
    }
    {
      const 行方不明 = r.道.filter((t) => !使った道.has(t) && !外した道.has(t));
      if (行方不明.length) problems.push("★git から来た道 " + r.道.length + "件のうち "
        + 行方不明.length + "件が、**使われも外れもしていません**(例: "
        + 行方不明.slice(0, 3).join(", ") + ")── ★★これは【黙って消えた】形です");
    }
    ...
    for (const 頭 of 場所.keys()) {
      ...
      並び.push({ 場所: 頭, 日: Math.floor((Date.now() / 1000 - t) / 86400) });
    }
    並び.sort((a, b) => b.日 - a.日);
    if (出すか) {
...
for (const n of 計器) console.log('  計 ' + n);   /* ★計器は ✓ で出さない(27.0) */
 ...
 *   ★**書いていなければ何も出ない**。 */
{
  ...
  if (予想を書く >= 0) {
    ...
    /* ★この物が【人の言葉を持つ】ことを、中身に宣言する(27.5、2026-09-03、@codex 16:17)。
     *   ★★@codex:「日付・判断・次回測定の基準という人の言葉が含まれる。消えると復元できない運用基準」。
     *   ★★★この物は 書き手を通るので台帳に載り、外すときは塊の物として消える(実測)。
     *   だから 保持一覧(=書き手が塊に【無い】物の一覧)には入らない ── 種類が違う。
     *   ★代わりに【中身に宣言】して、消す前に外す側が言えるようにする。
     *   ★★名前をべた書きしない ── この塊は手で並べた一覧を持たない。 */
    書き手.書く(ROOT, ".guardian/予想.json",
      JSON.stringify({ 人が書いた: true, 文, 数の行 }, null, 1) + String.fromCharCode(10), "check.mjs");
    console.log("");
    ...
    if (前 && Array.isArray(前.数の行)) {
      ...
      if (!消えた.length && !出た.length) {
        ...
        /* ★言い方を直した(2026-08-31、配布先の実測)。
         *   ★★予想の【文】は1行だが、流れ込む数は50行を超える。
         *   ★実測: 4つ予想を書いて、動いたのは【書かなかった2つ】だった。
         *   ★★★予想は「全部の数」ではなく「見に行きたい数」を書くもので、
         *     いちばんの材料は【書かなかった数が動いたとき】である。 */
        console.log("  ★予想と違っていたら、そこを見ること。"
