...
/* ★★★【均した指紋】── 改行を 揃えてから 数える(27.59、2026-09-05)。
 *
 *   ★なぜ 要るか: Windows の clone は CRLF に なる ── ★★同じ中身が 別の指紋に なる。
 *   ★★★実測(27.47): README.md は 手元 22,083B / git の中 21,818B ── 差 265 は 改行。
 *
 *   ★★そして ここが【1本】でなければ ならない ── ★27.59 で 私は
 *   install(CR を落とす)と 外す(落とさない)で 別々に 数え、★★★19件が
 *   「入れた時から 変わっている」と 出た。★中身は 1バイトも 変わっていないのに。
 *   ★= 同じ物を 2箇所で 別々に 数えると、必ず ずれる(★★この塊が 何度も 踏んだ形)。 */
const 均した指紋 = (中) => 指紋(
  (Buffer.isBuffer(中) ? 中.toString('utf8') : String(中)).split(String.fromCharCode(13)).join(''));
...
/* ★★★【現場の根を 探す】(27.69、2026-09-05、依頼主「人の手を 1つも 煩わすな」)。
 *
 *   ★なぜ 要るか: 受領証(pull の 由来)は これまで【束の 中】に 在った ──
 *   ★★束は 配る側の 目録と 1点も 違わない事で 所有を 証明するので、
 *   ★★★束の中に【配る時に 無い物】が 生まれると、その証明が 立たなくなる。
 *   ★人に「退避してから もう一度」と 頼む形に なっていた ── ★★それを 無くす。
 *
 *   ★★探し方は selfcheck の 宣言を読む() と 同じ形に 揃える(★写しを 増やさない為に
 *   ここを 正本に する):塊は 根の直下にも tools/ の下にも 置かれ得るので 決め打ちしない。
 *   ★★★見つからなければ null ── 正本自身や、まだ 導入していない現場が それに 当たる。 */
function 現場の根(塊の道) {
  const 候補 = [path.join(塊の道, '..'), path.join(塊の道, '..', '..'), process.cwd()];
  for (const d of 候補) {
    try { if (fs.existsSync(path.join(d, 'guardian.config.json'))) return path.resolve(d); } catch (_) {}
  }
  return null;
}

module.exports = { 書く, 追記, 登録, 更新, 書き換える, 指紋, 均した指紋, 親を作る, 台帳の道, 台帳の相対, 走行中の種類, 現場の根 };
