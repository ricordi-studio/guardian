...
/* ★【消す前に】読む(27.5、2026-09-03)── ★★実測: 一覧は消した後に出るので、
 *   ★★★本番では読めず、警告は【下見でしか出なかった】。
 *   下見だけ緑になる形は、この塊が禁じている「見えない失敗」そのもの。
 *   ★だから物に持たせて、消えた後でも言えるようにする。 */
for (const c of 消す) {
  try {
    if (c.種類 === 'ファイル' && c.rel.endsWith('.json'))
      c.人の言葉 = JSON.parse(fs.readFileSync(c.道, 'utf8')).人が書いた === true;
  } catch (_) { /* 読めない物は、ただの消す物として扱う */ }
}
