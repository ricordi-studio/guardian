...
if (走査だけ) {
  ...
  const 歩く = (相対) => {
    ...
    for (const e of es) {
      ...
      const 印のどれか = t.includes('<!-- guardian:begin') || t.includes('<!-- guardian:end');
      if (印のどれか) 丸ごと不可.add(rel);
...
if (判定 === 'CONFLICT') {
  ...
  /* ★束(①)を 数に入れて言う(27.38、2026-09-04、@claude が PASS の現場で 実測)。
 *
 *   ★★事故: PASS のとき「この一覧に残っているのは③(現場の物)だけです」と言っていた ──
 *   ★★★だが ①の束(guardian/ 33点)は 残っている。**言っている事と している事が 違う**。
 *
 *   ★「残り 0 件」は【台帳が知る物】の話、「③だけ」は【retained 全体】の話 ──
 *   ★★1つの段落で 範囲が 2つ 混ざっていた(★★★今日 何度も出た「1つの言葉が2つの意味」)。
 *
 *   ★依頼主の言葉は「残滓ゼロで外せる形」── ★★束が 残る事は、そこに 掛かる。
 *   ★★★道具は 束を 消さない(消すのは 人)。だから【言う】。 */
  /* ★残る束は【ディスクを見る】── retained からは 出ない(27.38b、押す前に叩いて分かった)。
 *   ★★retained は【走査した集合】から出る。★★★走査は guardian/ を「束の中」として
 *   見ていない側に入れるので、retained に 束は 残らない。
 *   ★だから「残っているか」は 走査の結果ではなく **現物** に 聞く。 */
  const 残る束 = (() => {
    const 出 = [];
    try {
  ...
  if (残る束.length) {
    ...
    console.log('  ★ですが【塊の束】が ' + 残る束.length + '件 残っています ── '
      + 'この道具は 束を 消しません(★★フォルダごと消すのは 人の手です)。');
    console.log('  ★★★残滓ゼロにするなら、確かめてから 手で消してください: '
      + 残る束.join(' / '));
  } else {
