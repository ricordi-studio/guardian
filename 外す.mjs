...
if (区間の印 == null) {
  console.log('★install の宣言(区間の印)が読めませんでした ── '
    + '★★区間は【接頭辞】で照合します(matchMode=prefix)。'
    + String.fromCharCode(10) + '  ★★★この照合では、別の道具が書いた区間と見分けが付きません ── '
...
for (const x of 項) {
  ...
  if (x.種類 === 'ファイル') {
    ...
    const 始 = 全文で当てた ? 中.indexOf(区間の印) : 中.indexOf(接頭);
    const 終印 = '<!-- guardian:end -->';
    const 終 = 始 < 0 ? -1 : 中.indexOf(終印, 始);   /* ★始より後ろの end を取る(前の対を掴まない) */
    if (始 < 0 || 終 < 始) {
      /* ★病名を1対1で当てる ── ★★接頭辞の有無だけで理由を決めない(27.18)。 */
      const 訳 = (!全文で当てた)
        ? '(★install の宣言が読めなかったので【接頭辞】で照合しました(matchMode=prefix)── '
          + '★★この照合では、別の道具が書いた区間と見分けが付きません)'
        : (自分の印が在る && 始 >= 0)
