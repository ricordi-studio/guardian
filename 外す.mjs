...
const 帳面の道 = path.join(ROOT, '.guardian', '撤去帳面.json');
const 帳面 = (() => {
  try {
...
if (帳面) {
  const 帳 = 帳面.身元 || {};
  const 合う = 台帳の身元 && 台帳の身元.取れた && String(帳.dev) === String(台帳の身元.dev) && String(帳.ino) === String(台帳の身元.ino);
  if (!合う) {
...
if (実行 && !依存衝突.length) {
  帳面を書く(消す.map((c) => c.rel), 済み, !!(束も消す || (帳面 && 帳面.証明済み)));
  for (const c of 消す) {
    try {
      if (c.種類 === 'ファイル') {
      ...
      済み.push(c.rel);
      帳面を書く(消す.map((x) => x.rel), 済み, !!(束も消す || (帳面 && 帳面.証明済み)));
    } catch (e) {
...
if (実行 && 判定 === 'PASS') {
  try {
    /* ★帳面は【全部 済んだ】時にだけ 消す(27.85)── ★★残っていれば 次の走行が 続きを やる。 */
    fs.rmSync(帳面の道, { force: true });
