 ...
 *
  ...
  const 強い順 = ['雛形と一致', '塊が置く道(中身は違う)', '塊が置く道', '印', '昔の置き場', '命令', '場所だけ', '名前だけ'];
  ...
  const 強い証拠 = new Set(['雛形と一致', '塊が置く道', '塊が置く道(中身は違う)', '印', '昔の置き場']);
  ...
  if (名指し引数 >= 0) {
    ...
    for (const r of 名) {
      ...
      if ((見た.get(r) || []).some((e) => e.型 === '印')) {
        断った.push(r + '(★この道には区間(guardian:begin/end)が在ります ── '
          + '★★丸ごとは消せません。現場の文まで一緒に消えるので。'
          + '★★★区間だけを外すなら、台帳の道 `--外す` を使ってください)'); continue;
      }
