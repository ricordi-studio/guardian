...
/* ★★★掃いてよい物の 一覧は【install が 書いた 控え】を 先に 使う(27.94、@guardian 23:40)。
 *   ★帳面は 撤去の 途中で 書く 紙 ── ★★人が 差し替えやすい。
 *   ★★★控え(.guardian/束の控え.json)は install が 書き、目録と 1点も 違わない事を
 *   確かめた 物 ── ★同じ嘘を つくには 2つ 揃えて 細工する 必要が 在る。
 *   ★★控えが もう 無い(撤去が 進んでいる)時だけ、帳面の 写しに 落ちる。 */
const 掃く印 = new Map(束の印の対.length ? 束の印の対
  : ((再開中 && 帳面 && Array.isArray(帳面.束の印)) ? 帳面.束の印.filter((x) => Array.isArray(x)) : []));
